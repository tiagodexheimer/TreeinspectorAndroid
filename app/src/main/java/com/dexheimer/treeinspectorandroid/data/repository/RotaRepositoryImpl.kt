...
constructor(
    ...
    override suspend fun getRotas(): Result<List<Rota>> =
        ...
        return withContext(Dispatchers.IO) {
            ...
            try {
                ...
                if (response.isSuccessful && response.body() != null) {
                    ...
                    val demandasEntities =
                            data.demandas.map { dto ->
                                mergeWithLocalStatus(dto.toEntity(rotaId), statusLocalMap)
                            }

                    // PASSO 3: Inserir (Com OnConflictStrategy.REPLACE no DAO, isso atualiza o
                    // registro)
                    rotaDao.insertRota(data.rota)
                    demandaDao.insertAll(demandasEntities)

                    Result.success(
                            Pair(data.rota.toDomain(), demandasEntities.map { it.toDomain() })
                    )
                } else {
    ...
    /**
     * Mescla o status local com a entidade da API. Se a demanda foi concluída localmente, preserva
     * o status local.
     */
    private fun mergeWithLocalStatus(
            entity: com.dexheimer.treeinspectorandroid.data.local.DemandaEntity,
    ...
    override suspend fun sincronizarRotas(): Result<Unit> =
            withContext(Dispatchers.IO) {
                ...
                return@withContext runCatching {
                            ...
                            // CORREÇÃO EXCLUSÃO: Remove rotas locais que não estão mais no
                            // servidor.
                            ...
                            for (rota in rotas) {
                                ...
                                val statusLocalMap =
                                        demandasLocais.associate { it.id to it.statusVistoria }
                                ...
                                if (detalheResponse.isSuccessful && detalheResponse.body() != null
                                    ...
                                    val demandasEntities =
                                            dados.demandas.map { dto ->
                                                mergeWithLocalStatus(
                                                        dto.toEntity(rota.id),
                                                        statusLocalMap
                                                )
                                            }
                            ...
                            for (tipo in tiposDeDemandaParaBaixar) {
                                try {
                                    ...
                                    if (formResponse.isSuccessful && formResponse.body() != null) {
                                        ...
                                        Log.d(
                                                "Sync",
                                                "Formulário para '$tipo' baixado com sucesso."
                                        )
                                    }
                                } catch (e: Exception) {
                        ...
                        .fold(
                                ...
                                onFailure = { e ->
                                    Log.e(
                                            "Sync",
                                            "ERRO FATAL DURANTE A SINCRONIZAÇÃO. Motivo: ${e.message}",
                                            e
                                    )
                                    // Converte Throwable em uma Exception amigável
                                    Result.failure(
                                            Exception(
                                                    "Erro fatal na sincronização: ${e.message ?: "Desconhecido"}"
                                            )
                                    )
                                }
                        )

...
import com.dexheimer.treeinspectorandroid.presentation.demandas.CriarDemandaActivity
...
class RotaDetalheActivity : AppCompatActivity() {
    ...
    private lateinit var btnCriarDemandaCampo: Button
    ...
    private val demandaListLauncher =
            registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
                if (result.resultCode == Activity.RESULT_OK) {
    ...
    private val requestPermissionLauncher =
            registerForActivityResult(ActivityResultContracts.RequestMultiplePermissions()) {
                ...
                if (permissions[Manifest.permission.ACCESS_FINE_LOCATION] == true ||
                    ...
                    Toast.makeText(
                                    this,
                                    "Permissão de localização necessária para mostrar a rota.",
                                    Toast.LENGTH_LONG
                            )
                            .show()
                }
    ...
    private fun setupUI() {
        ...
        btnCriarDemandaCampo = findViewById(R.id.btnCriarDemandaCampo)
        ...
        btnCriarDemandaCampo.setOnClickListener { abrirCriarDemanda() }
    }
    ...
    private fun setupLocationOverlay() {
        ...
        locationOverlay?.runOnFirstFix {
            runOnUiThread {
                viewModel.uiState.value.let { state -> atualizarMapa(state.demandasPendentes) }
            }
        }
        mapView.invalidate()
    }
        ...
        lifecycleScope.launch {
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                viewModel.uiState.collect { state ->
                    ...
                    if (state.error != null)
                            Toast.makeText(this@RotaDetalheActivity, state.error, Toast.LENGTH_LONG)
                                    .show()
    ...
    private fun atualizarMapa(demandas: List<Demanda>) {
        ...
        demandas.forEachIndexed { index, demanda ->
            if (demanda.lat != null && demanda.lng != null) {
                ...
                val iconRes =
                        if (index == 0) R.drawable.ic_marker_red else R.drawable.ic_marker_blue
            ...
            rotaJob =
                    lifecycleScope.launch {
                        ...
                        if (pontosRota.isNotEmpty()) {
                            ...
                            // FALHA: Desenha a linha reta novamente (Fallback) mas com a cor
                            // oficial
                            val linhaFallback =
                                    Polyline().apply {
                                        addPoint(userLocation)
    ...
    private suspend fun fetchRoutePoints(start: GeoPoint, end: GeoPoint): List<GeoPoint> =
                ...
                try {
                    ...
                    val request =
                            Request.Builder()
                                    ...
                                    .header(
                                            "User-Agent",
                                            "TreeInspectorAndroid/1.0"
                                    ) // IMPORTANTE: Header adicionado
                                    .build()
                    ...
                    if (response.isSuccessful) {
                        ...
                        Log.e(
                                "RotaDetalhe",
                                "Erro API OSRM: ${response.code} - ${response.message}"
                        )
                    }
                } catch (e: Exception) {
    ...
    private fun abrirCriarDemanda() {
        val intent =
                Intent(this, CriarDemandaActivity::class.java).apply { putExtra("ROTA_ID", rotaId) }
        startActivity(intent)
    }

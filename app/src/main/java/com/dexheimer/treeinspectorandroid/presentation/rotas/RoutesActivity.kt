...
class RoutesActivity : AppCompatActivity() {
	...
	private fun setupUI() {
		...
		supportActionBar?.setDisplayHomeAsUpEnabled(true) // Para voltar (logout)

		swipeRefresh = findViewById(R.id.swipeRefresh)
		val recyclerView = findViewById<androidx.recyclerview.widget.RecyclerView>(R.id.recyclerRotas)
	...
	override fun onOptionsItemSelected(item: MenuItem): Boolean {
		return when(item.itemId) {
			...
			R.id.action_sync_status -> {
				...
				if (pendentes > 0) {
					Toast.makeText(this, "Existem $pendentes vistorias aguardando envio. Arraste para baixo para sincronizar.", Toast.LENGTH_LONG).show()
				} else {
	...
	private fun updateSyncIcon(pendingCount: Int) {
		syncMenuItem?.let { item ->
			if (pendingCount > 0) {
				item.icon = ContextCompat.getDrawable(this, R.drawable.ic_cloud_upload) // Ícone de "Falta subir" (Amarelo/Aviso)
				// Se quiser mudar a cor via código:
				// item.icon?.setTint(Color.YELLOW)
			} else {
				item.icon = ContextCompat.getDrawable(this, R.drawable.ic_cloud_done) // Ícone de "Ok" (Verde/Branco)
			}
		}
	}
}